...
func NewEmptyRound4(g curve.Curve, pl *pool.Pool) *round4 {
	return &round4{
		round3:             NewEmptyRound3(g, pl),
		MessageBroadcasted: make(map[party.ID]bool),
		MessagesForwarded:  make(map[party.ID]bool),
	}
}
func (r *round4) Serialize() (ser []byte, err error) {
	rs := round4Serialized{
		ChainKey:           r.ChainKey,
		RID:                r.RID,
	...
	rs.Round3, err = r.round3.Serialize()
	if err != nil {
		return nil, err
	}
...
func (r *round4) Deserialize(data []byte) error {
	var rs round4Serialized
	if err := json.Unmarshal(data, &rs); err != nil {
		return err
	}

	if err := r.round3.Deserialize(rs.Round3); err != nil {
		return err
	}

	r.ChainKey = rs.ChainKey
	r.RID = rs.RID
	r.MessageBroadcasted = rs.MessageBroadcasted
	r.MessagesForwarded = rs.MessagesForwarded
