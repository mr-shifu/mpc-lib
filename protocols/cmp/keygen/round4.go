...
func (r *round4) StoreBroadcastMessage(msg round.Message) error {
	...
	if !body.Mod.Verify(zkmod.Public{N: ped.PublicKeyRaw().N()}, r.HashForID(from), r.Pool) {
		return errors.New("failed to validate mod proof")
	}

	// verify zkprm
	if !body.Prm.Verify(zkprm.Public{Aux: ped.PublicKeyRaw()}, r.HashForID(from), r.Pool) {
...
func (r *round4) VerifyMessage(msg round.Message) error {
	...
	if !paillierKey.VerifyZKFAC(body.Fac, zkfac.Public{
		...
		// if !body.Fac.Verify(zkfac.Public{N: paillierKey.PublicKey().ParamN(), Aux: ped.PublicKeyRaw()}, r.HashForID(from)) {
